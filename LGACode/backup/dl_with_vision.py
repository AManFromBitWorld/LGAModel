...
filepath = '/Users/mingyangzhong/Desktop/dl/orgi/'
class DataLoader():
	def __init__(self,batch_size):
		self.batch_size = batch_size

		train = np.genfromtxt(filepath+'user-video-pair-train.txt',dtype='|S', delimiter = ',')
		test = np.genfromtxt(filepath+'user-video-pair-text.txt',dtype='|S', delimiter = ',')
		...
		for record in data:
			self.matrix[record[0]-1, record[1]-1] = 1
	...
	def val_data(self):
		...
		test = np.genfromtxt(filepath+'user-video-pair-text.txt',delimiter = ',').astype(np.int32)
...
def test(args, k=5):
	...
	with tf.Session() as sess:
		...
		for uid, row in enumerate(mat):
			...
			print prediction.shape, uid, len(mat)
...
def main():
	...
	parser.add_argument("--vision_flag", type=bool, default = False, help = 'enable vision features')
	parser.add_argument("--text_flag", type = bool, default = False, help = 'enable text features')
