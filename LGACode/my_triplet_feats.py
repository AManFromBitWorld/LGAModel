...
############################################################auto add layers##########################################
		if vision != None or text != None:
			...
			with tf.variable_scope("hiden1"):
				w1 = tf.get_variable('w1',[vec_size,128], initializer = tf.random_normal_initializer(stddev = 0.02))
				b1 = tf.get_variable('b1', [128], initializer = tf.constant_initializer(0.0))
				hidden = tf.matmul(vec,w1)+b1

			with tf.variable_scope('hiden2'):
				w2 = tf.get_variable('w2',[128,64], initializer = tf.random_normal_initializer(stddev=0.02))
				b2 = tf.get_variable('b2', [64], initializer = tf.constant_initializer(0.0))
				output = tf.matmul(hidden, w2)+b2

			with tf.variable_scope('score'):
				w3 = tf.get_variable('w3',[64,1], initializer = tf.random_normal_initializer(stddev=0.02))
...
################################################### auto add layers ###############################################
  ...
  #    				shape = [hidden_size[i-1],size]
  #   			w = tf.get_variable('w'+str(i), shape, initializer = tf.random_normal_initializer(stddev=0.02))
  #   			b = tf.get_variable('b'+str(i), [size], initializer = [tf.constant_initializer(0.0)]tf.constant_initializer(0.0))
  #  				 # print i, prev_layer.get_shape(), w.get_shape()
  #  				hidden_layer.append(tf.matmul(hidden_layer[-1], w)+b)
 	# 	return hidden_layer[-1]
		...
		# 		else:
		# 			shape = [hidden_size[i-1],size]
		# 		locals()['w%i'%i] = tf.get_variable('w'+str(i), shape, initializer = tf.random_normal_initializer(stddev=0.02))
		# 		locals()['b%i'%i] = tf.get_variable('b'+str(i), [size], initializer = tf.constant_initializer(0.0))
		# 		# print i, prev_layer.get_shape(), w.get_shape()
		# 		hidden_layer.append(tf.matmul(hidden_layer[-1], locals()['w%i'%i])+locals()['b%i'%i])
		# return hidden_layer[-1]
...
# filepath = '/Users/mingyangzhong/Desktop/dl/'
filepath = '/home/jingwei/Desktop/dl/'
class DataLoader():
	def __init__(self,batch_size):
		self.batch_size = batch_size

		train = np.genfromtxt(filepath+'user-video-pair-train.txt',dtype='|S', delimiter = ',')
		# train = np.genfromtxt(filepath+'train.txt',dtype='|S', delimiter = ',')
		test = np.genfromtxt(filepath+'user-video-pair-test.txt',dtype='|S', delimiter = ',')
		# test = np.genfromtxt(filepath+'test.txt',dtype='|S', delimiter = ',')
	...
	def val_data(self):
		...
		test = np.genfromtxt(filepath+'user-video-pair-test.txt',delimiter = ',').astype(np.int32)
		# test = np.genfromtxt(filepath+'test.txt',delimiter = ',').astype(np.int32)
