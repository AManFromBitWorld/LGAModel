...
class DataLoader():
	...
	def next_batch(self):
		...
		return self.matcoo.row[begin:end].reshape(-1,1), \
				self.matcoo.col[begin:end].reshape(-1,1), self.sequences[self.matcoo.col[begin:end]], \
				nitem.reshape(-1,1), self.sequences[nitem]
		#return self.matcoo.row[begin:end].reshape(-1,1), \
	...
	with tf.Session() as sess:
		...
		for e in range(args.nb_epochs):
			...
			for b in range(total_batch):
				user,pitem, ptext, nitem, ntext = data_loader.next_batch()
