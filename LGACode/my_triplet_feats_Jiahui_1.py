...
class Model():
    ...
    def __init__(self, args, infer=False):
        ...
        if args.vision_flag:
            self.pvision = tf.placeholder(
                tf.float32, shape=[args.batch_size, args.vision_feat_size])
            self.nvision = tf.placeholder(
                tf.float32, shape=[args.batch_size, args.vision_feat_size])
        if args.text_flag:
            self.ptext = tf.placeholder(
                tf.int32, shape=[args.batch_size, args.seq_maxlen])
            self.ntext = tf.placeholder(
                tf.int32, shape=[args.batch_size, args.seq_maxlen])

            text_embed = Embedding(
                input_dim=args.vocab_size, output_dim=args.text_embed_size, input_length=args.seq_maxlen)

            ptext_embed = text_embed(self.ptext)
            ntext_embed = text_embed(self.ntext)

            ptext_rnn = BiRNN(ptext_embed, args.rnn_size,
                              embed_size=args.text_embed_size, n_steps=args.seq_maxlen)
            ntext_rnn = BiRNN(ntext_embed, args.rnn_size,
                              embed_size=args.text_embed_size, n_steps=args.seq_maxlen, reuse=True)
        ...
        u_embed = Embedding(input_dim=args.n_user,
                            output_dim=args.embedding_size, input_length=1)
        i_embed = Embedding(input_dim=args.n_item,
                            output_dim=args.embedding_size, input_length=1)

        user_latent = tf.reshape(u_embed(self.user), [
                                 args.batch_size, args.embedding_size])
        pitem_latent = tf.reshape(i_embed(self.pitem), [
                                  args.batch_size, args.embedding_size])
        nitem_latent = tf.reshape(i_embed(self.nitem), [
                                  args.batch_size, args.embedding_size])
        ...
        if args.vision_flag == False and args.text_flag == False:
            ...
            self.pscore = self.scoring(
                user_latent, pitem_latent, text=ptext_rnn)
            self.nscore = self.scoring(
                user_latent, nitem_latent, text=ntext_rnn, reuse=True)
        elif args.text_flag == False:
            self.pscore = self.scoring(
                user_latent, pitem_latent, vision=self.pvision)
            self.nscore = self.scoring(
                user_latent, nitem_latent, vision=self.nvision, reuse=True)
        else:
            self.pscore = self.scoring(
                user_latent, pitem_latent, vision=self.pvision, text=ptext_rnn)
            self.nscore = self.scoring(
                user_latent, nitem_latent, vision=self.nvision, text=ntext_rnn, reuse=True)

        self.loss = - \
            tf.reduce_mean(tf.log(tf.nn.sigmoid(self.pscore - self.nscore)))
        ...
        if vision != None or text != None:
            ...
            for i, size in enumerate(hidden_size):
                with tf.variable_scope('hidden_vision_text'+str(i)):
                    ...
                    w = tf.get_variable(
                        'w'+str(i), shape, initializer=tf.random_normal_initializer(stddev=0.02))
                    b = tf.get_variable(
                        'b'+str(i), [size], initializer=tf.constant_initializer(0.0))
...
filepath = '/home/jingwei/Desktop/LGA/dl/'
...
class DataLoader():
    ...
    def __init__(self, batch_size):
        ...
        train = np.genfromtxt(
            filepath + 'user-video-pair-train.txt', dtype='|S', delimiter=',')
        test = np.genfromtxt(
            filepath + 'user-video-pair-test.txt', dtype='|S', delimiter=',')
    ...
    def process_text(self):
        ...
        tweets = [line.strip().split(" ## ") for line in open(
            'exp_sameTweetsWithoutID(Video_Tweets).txt').readlines()]
        tweetsDict = dict((url, tweet) for url, tweet in tweets)

        filtered_tweets = []
        for item in sorted_videoId:
            url = item[0]
        ...
        self.sequences = [[self.word2idx[word] for word in sent.lower(
        ).split() if word in self.word2idx] for sent in filtered_tweets]
    ...
    def sample_negative(self, u):
        ...
        for i, user in enumerate(u):
            if user not in self.user_train:
                ...
                filtered_index = list(
                    set(range(self.n_item)).difference(set(self.user_train[user])))
    ...
    def next_batch(self):
        ...
        #       pitem.append(b)
        #       nitem.append(a)
        # return np.array(user).reshape(-1,1), np.array(pitem).reshape(-1,1),
        # np.array(nitem).reshape(-1,1)
    ...
    def val_data(self):
        ...
        test = np.genfromtxt(
            filepath+'user-video-pair-text.txt', delimiter=',').astype(np.int32)
        ...
        # return test[:,0].reshape(-1,1)-1, test[:,1].reshape(-1,1)-1,
        # test[:,2].reshape(-1,1)
...
def test(args, k=5):
    ...
    with tf.Session() as sess:
        ...
        for uid, row in enumerate(mat):
            ...
            filtered_index = list(set(range(len(row))).difference(
                set(data_loader.user_train[uid])))

            if len(np.unique(row[filtered_index])) == 1:
                continue

            auc.append(roc_auc_score(
                row[filtered_index], prediction[filtered_index]))
        ...
        print("roc_auc_score:{}. precision@{}:{}".format(
            np.mean(auc), k, np.mean(precision)))
        ...
        for e in range(args.nb_epochs):
            ...
            for b in range(total_batch):
                ...
                feed = {model.user: user,
                        model.pitem: pitem, model.nitem: nitem}
                if args.vision_flag:
                ...
                loss, _ = sess.run(
                    [model.loss, model.train_op], feed_dict=feed)

                sys.stdout.write(
                    "\r {}/{}, {}/{}. loss:{}".format(e, args.nb_epochs, b, total_batch, loss))
                sys.stdout.flush()

                if (e*args.nb_batches+b) % 1000 == 0 or (e == args.nb_epochs-1 and b == args.nb_batches-1):
                    saver.save(sess, './checkpoint/'+"model.ckpt",
                               global_step=e*args.nb_batches+b)


def main():
    start = time.time()
    parser = argparse.ArgumentParser()
    parser.add_argument("--n_user", type=int, default=943,
                        help="number of users")
    parser.add_argument("--n_item", type=int, default=1682,
                        help='number of items')
    parser.add_argument("--batch_size", type=int, default=64,
                        help='batch size for training')
    parser.add_argument("--nb_epochs", type=int,
                        default=100, help='number of epochs')
    parser.add_argument("--nb_batches", type=int,
                        default=100, help='number of batches')
    parser.add_argument('--embedding_size', type=int,
                        default=64, help='embedding size of latent factors')
    parser.add_argument("--vision_feat_size", type=int,
                        default=19, help='size of movie vision features')
    parser.add_argument("--vision_flag", type=bool,
                        default=False, help='enable vision features')
    parser.add_argument("--text_flag", type=bool,
                        default=False, help='enable text features')
    parser.add_argument("--seq_maxlen", type=int, default=100,
                        help='maximum length of sentence')
    parser.add_argument("--vocab_size", type=int, default=100,
                        help='vocabulary size of tweets')
    parser.add_argument("--text_embed_size", type=int,
                        default=128, help='vocabulary size of tweets')
    parser.add_argument("--rnn_size", type=int, default=128,
                        help='size of rnn output layers')
    parser.add_argument("--train_test", type=str,
                        default='train', help='size of rnn output layers')
